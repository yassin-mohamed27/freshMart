...
export default async function BrandProductsPage({
  ...
  {/* Header نفس الديزاين */}
  <div className="relative overflow-hidden rounded-3xl border border-green-200 bg-white p-6 sm:p-8 shadow-sm">
    {/* تأثيرات ضبابية أخضر */}
    <div className="pointer-events-none absolute -top-24 -right-24 h-72 w-72 rounded-full bg-emerald-300/20 blur-3xl" />
    <div className="pointer-events-none absolute -bottom-28 -left-24 h-72 w-72 rounded-full bg-green-300/20 blur-3xl" />

    <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-4 min-w-0">
        {brand?.image ? (
          <div className="relative h-12 w-28 rounded-2xl border border-green-200 bg-green-50 p-2 shadow-sm">
            <Image
              src={brand.image}
              alt={brand.name}
              fill
          ...
          <div className="h-12 w-28 rounded-2xl border border-green-200 bg-green-50" />
        )}
        ...
        <div className="min-w-0">
          ...
          <h1 className="mt-1 text-2xl sm:text-3xl font-extrabold tracking-tight text-green-900 line-clamp-1">
      ...
      <Link
        href="/brands"
        className="shrink-0 inline-flex items-center justify-center rounded-xl border border-green-200 bg-green-50 px-4 py-2 text-sm font-semibold text-green-800 hover:bg-green-100 transition"
      >
        Back to brands
      </Link>
    </div>
