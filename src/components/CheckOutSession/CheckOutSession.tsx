...
export default function CheckOutSession({ cartId }: { cartId: string }) {
  ...
  <DialogContent
    className={[
      ...
      "w-[calc(100vw-16px)] rounded-3xl sm:w-[min(980px,calc(100vw-48px))]",
  ...
  >
                ...
                {addresses.map((a) => (
                  <button
                    ...
                    className={[
                      ...
                      selectedId === a._id
                        ? "border-emerald-500/60 bg-slate-950 ring-1 ring-emerald-500/30"
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className="inline-flex size-4 items-center justify-center rounded-full border border-slate-700">
                        <span className={["size-2 rounded-full", selectedId === a._id ? "bg-emerald-400" : "bg-transparent"].join(" ")} />
                      </span>
                      <div className="text-sm font-extrabold text-slate-100 line-clamp-1">{a.name || "Address"}</div>
          ...
          <div className="rounded-3xl border border-slate-800 bg-slate-900/20 p-4 sm:p-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-slate-400">{selectedId ? "Using saved address" : "New address"}</div>
              </div>
              <span className={`text-[11px] font-bold ${selectedId ? "text-emerald-300" : "text-slate-400"}`}>
                {selectedId ? "Selected" : "Editable"}
              </span>
            </div>
            ...
            <FieldGroup className="gap-4 sm:gap-5 mt-4">
              ...
              <Field>
                ...
                <div className="relative">
                  ...
                  <Input ref={city} id="city" name="city" defaultValue={selectedAddress?.city || ""} placeholder="Cairo"
                    className="h-12 rounded-2xl pl-10 bg-slate-900/60 border-slate-800 text-slate-100 placeholder:text-slate-500 focus-visible:ring-2 focus-visible:ring-emerald-400/60" />
                </div>
              </Field>

              {/* Details */}
              <Field>
                <Label htmlFor="details" className="text-slate-300">Address details</Label>
                <div className="relative">
                  <Home className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 size-4 text-slate-500" />
                  <Input ref={details} id="details" name="details" defaultValue={selectedAddress?.details || ""} placeholder="Maadi, Street, Building..."
                    className="h-12 rounded-2xl pl-10 bg-slate-900/60 border-slate-800 text-slate-100 placeholder:text-slate-500 focus-visible:ring-2 focus-visible:ring-emerald-400/60" />
                </div>
              </Field>
              ...
              <Field>
                ...
                <div className="relative">
                  ...
                  <Input ref={phone} id="phone" name="phone" defaultValue={selectedAddress?.phone || ""} placeholder="01xxxxxxxxx"
                    className="h-12 rounded-2xl pl-10 bg-slate-900/60 border-slate-800 text-slate-100 placeholder:text-slate-500 focus-visible:ring-2 focus-visible:ring-emerald-400/60" />
                </div>
              </Field>
            </FieldGroup>
            ...
            {/* Payment */}
            <div className="rounded-3xl border border-slate-800 bg-slate-900/20 p-4 sm:p-5 mt-4">
              ...
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {/* Cash */}
                <button type="button" onClick={payCash} disabled={isLoading}
                  className="group rounded-3xl border p-4 text-left transition hover:bg-emerald-600/40 hover:border-emerald-500 active:scale-[0.99] disabled:opacity-60">
                  <div className="flex items-center justify-between">
                    <div className="grid size-10 place-items-center rounded-2xl bg-emerald-900/30 ring-1 ring-emerald-800">
                      {loadingType === "cash" ? <Loader2 className="size-4 animate-spin text-emerald-200" /> : <Banknote className="size-5 text-emerald-300" />}
                    </div>
                    <span className="text-[10px] font-bold text-slate-200 group-hover:text-white">FAST</span>
                  </div>
                ...
                {/* Card */}
                <button type="button" onClick={payWithCard} disabled={isLoading}
                  className="group rounded-3xl border p-4 text-left transition hover:bg-blue-600/30 hover:border-blue-500 active:scale-[0.99] disabled:opacity-60">
                  <div className="flex items-center justify-between">
                    <div className="grid size-10 place-items-center rounded-2xl bg-blue-900/30 ring-1 ring-blue-800">
                      {loadingType === "card" ? <Loader2 className="size-4 animate-spin text-blue-200" /> : <CreditCard className="size-5 text-blue-300" />}
                    </div>
                    <span className="text-[10px] font-bold text-slate-200 group-hover:text-white">SECURE</span>
                  </div>
              ...
              <p className="mt-4 text-[11px] text-slate-500">By placing your order, you agree to our terms and privacy policy.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="sticky bottom-0 z-20 border-t border-slate-800 bg-slate-950/80 backdrop-blur px-4 sm:px-6 py-4">
      <div className="flex items-center gap-3">
        <DialogClose asChild>
          <Button variant="outline" disabled={isLoading || savingAddress}
